...
function arraySortInfo(inputArray){
    if (inputArray.some(el => typeof el !== 'number')){
        return "Деякі значення не є числами";
    }

    if (inputArray.every((el, index) => index === 0 || el >= inputArray[index - 1])){
        return "Масив відсортовано за збільшенням"
    }

    if (inputArray.every((el, index) => index === 0 || el <= inputArray[index - 1])){
        return "Масив відсортовано за зменшенням"
    }
